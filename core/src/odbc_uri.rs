...
impl ODBCUri {
    ...
    async fn handle_no_uri(&mut self) -> Result<UserOptions> {
        ...
        client_options.hosts = vec![ServerAddress::parse(&server).map_err(Error::InvalidClientOptions)?];
        client_options.credential = Some(
            Credential::builder().username(user).password(pwd).build(),
        );
    ...
    fn finalize_client_options(
        ...
        if opts.credential.is_some() {
            ...
            let user = self
                .get_attribute(USER_KWS)
                .cloned()
                .ok_or_else(|| Error::InvalidUriFormat(format!(
                    "One of {:?} is required for a valid Mongo ODBC Uri",
                    USER_KWS
                )))?;
            let pwd = self
                .get_attribute(PWD_KWS)
                .cloned()
                .ok_or_else(|| Error::InvalidUriFormat(format!(
                    "One of {:?} is required for a valid Mongo ODBC Uri",
                    PWD_KWS
        ...
        let app_name = {
            ...
            Some(
                vec![base, odbc_app, mongo_app]
                    .into_iter()
                    .flatten()
                    .fold(String::new(), |acc, x| if acc.is_empty() { x } else { format!("{acc}|{x}") }),
            )
        };
